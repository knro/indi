...
/**
 * \class CCD
 * \brief Parent class for all camera drivers.
 *
 * The INDI::CCD class provides interfaces and functions that supports the implementation of concrete
 * INDI camera drivers with diverse features and capabilities.
 *
 * ## Capabilities
 * Camera capabilities define the feature set supported by the hardware camera device. The capabilities enables or disables
 * certain properties and features of the camera. For example, CCD_HAS_COOLER capability indicates that the camera
 * support TEC cooling that can be monitored and controlled via software. By declaring such capability, the respective temperature
 * control properties are defined to clients and any requests are dispatched to the child class which is expected to override the
 * SetTemperature virtual function to implement any necessary API calls to the camera's SDK to achieve the desired set point.
 *
 * The camera capabilities must be set to select which features are exposed to the clients.
 * SetCCDCapability() is typically set in the constructor or initProperties(), but can also be
 * called after connection is established with the camera, but must be called /em before returning
 * true in Connect(). Changing the capabilities at run-time is supported but should be used with caution.
 *
 * ## Guiding
 * Pulse guiding via ST4 port can be enabled in camera capabilities.
 *
 * ## Capture Format
 *
 * At least one capture format must be defined. The capture format specifies the native data type as generated by the camera (e.g. RAW or RGB).
 *
 * ## Encoder Format
 *
 * Before the image is transferred the the client, it is encoded to a suitable transport format. By default, the encoder format is FITS.
 * Supported formats include Native (send data as is) and XISF.
 *
 * ## Metadata
 * The class enable the ability to \e snoop  on telescope equatorial coordinates and record them in the FITS header before upload. It also
 * snoops Sky-Quality-Meter devices to record sky quality in units of Magnitudes-Per-Arcsecond-Squared
 * (MPASS) in the FITS header. Arbitrary Name/Value/Comment FITS header can be inserted by updating the
 * FITSHeaderTP property.
 *
 * ## Streaming
 * Support for streaming and recording is available and is handled by the StreamManager class.
 *
 * Developers need to subclass INDI::CCD to implement any driver for cameras within INDI.
 *
 * Data binary transfers are supported using two methods:
 * # INDI BLOBs: This is the and recommended default configuration. If FAST_BLOB is enabled then transfers of BLOB
 * within the same machine are written into buffers (shm or memfd) that are then exchanged by reference and shared & mmaped in the client.
 ...
 * The camera buffer data is protected by the m_BufferLock mutex. When reading the camera data
 ...
 * the same m_BufferLock mutex.
 *
 * \example CCD Simulator
 * \version 2.0
 * \author Jasem Mutlaq
 * \author Gerry Rozema
 * \note The class is called CCD for historical reasons. It is not limited to CCD cameras by any means.
 *
 */
class CCD : public DefaultDevice, GuiderInterface
{
    ...
    protected:
        ...
        char m_ExposureStartTime[MAXINDINAME];
        ...
        // Mount
        // Epoch of Date
        double m_MountJNowRA {0};
        double m_MountJNowDE {0};
        // J2000 Position
        double m_MountJ2000RA {0};
        double m_MountJ2000DE {0};
        bool m_MountJ2000Valid {false};
        // pier side, read from mount if available, set to -1 if not available
        // West = 0, East =1. No enum available
        int8_t m_MountPierSide {-1};
        ...
        // Rotator
        // Rotator Angle CW
        double m_RotatorAngle {-1};

        // Focuser
        // Absolute Position
        int32_t m_FocuserAbsolutePosition {-1};
        // Temperature (C)
        double m_FocuserTemperature {-1};
         ...
         */
        INDI::PropertyText ActiveDeviceTP {0};
        enum
        {
            ACTIVE_TELESCOPE,
            ACTIVE_ROTATOR,
        ...
        */
        ...
        // WCS
        // WCS Metadata
        INDI::PropertyNumber WCSMetadataNP {2};
        enum
        {
            WCS_ROTATION,
            WCS_PARITY
        ...
        std::chrono::system_clock::time_point m_FastExposureToggleStartup;
