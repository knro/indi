...
{
    // Initialize Filter Slot
    FilterSlotNP[0].fill("FILTER_SLOT_VALUE", "Filter", "%3.0f", 1, 12, 1, 1);
}
    ...
    if (dev && !strcmp(dev, m_defaultDevice->getDeviceName()) && !strcmp(name, "FILTER_NAME"))
    {
        ...
        if (loadingFromConfig)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                INDI::WidgetText oneFilter;
                oneFilter.fill(filterName, filterLabel, texts[i]);
                FilterNameTP.push(std::move(oneFilter));
            }

            FilterNameTP.fill(m_defaultDevice->getDeviceName(), "FILTER_NAME", "Filter", FilterSlotNP.getGroupName(), IP_RW, 60,
                              IPS_IDLE);
...
void FilterInterface::SelectFilterDone(int f)
{
    ...
    // Tell the clients we are done, and filter is now useable
    FilterSlotNP.apply();
}
...
void FilterInterface::generateSampleFilters()
{
    ...
    for (int i = 0; i < MaxFilter; i++)
    {
        ...
        INDI::WidgetText oneFilter;
        oneFilter.fill(filterName, filterLabel, i < 8 ? filterDesignation[i] : filterLabel);
        FilterNameTP.push(std::move(oneFilter));
    }
    ...
    for (root = nextXMLEle(fproot, 1); root != nullptr; root = nextXMLEle(fproot, 0))
    {
        ...
        if (!strcmp("FILTER_NAME", rname))
        {
            ...
            for (oneText = nextXMLEle(root, 1); oneText != nullptr; oneText = nextXMLEle(root, 0))
            {
                ...
                INDI::WidgetText oneFilter;
                oneFilter.fill(filterName, filter, filter);
                FilterNameTP.push(std::move(oneFilter));
                counter++;
            }
