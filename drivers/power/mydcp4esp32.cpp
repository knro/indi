...
bool MyDCP4ESP::SetDewPort(size_t port, bool enabled, double dutyCycle)
{
    ...
    if (port == 2) // Channel 3 is port 2 (0-based index)
    {
        ...
        {
            LOG_INFO("Dew port duty cycle can only be set for Channel 3 when in Manual mode.");
        }
    }
    else
    {
        LOGF_INFO("Dew port duty cycle cannot be set Channel %zu.", port + 1);
    }
    return false;
}
...
bool MyDCP4ESP::SetAutoDewEnabled(size_t port, bool enabled)
{
    ...
    // and cannot be disabled except for Channel 3 which can be set to manual if it
    // has a probe attached.
    if (port == 2)
    {
